...
def build_cues(pause: float, audio_preroll: float, width: int) -> list[tuple[float, float, str]]:
    slides = combine_slides(ROOT)
    cues: list[tuple[float, float, str]] = []
    cursor = 0.0
    for index, slide in enumerate(slides):
        duration = wav_duration(ROOT / "narration" / "audio" / f"{index:03d}.wav")
        cues.append((cursor + audio_preroll, cursor + audio_preroll + duration, wrap_text(slide.narration, width)))
...
def main() -> None:
    ...
    cues = build_cues(args.pause, args.audio_preroll, args.wrap_width)
