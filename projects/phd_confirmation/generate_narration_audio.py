...
SMOKE_MODEL_PATH = ROOT / "narration_smoke" / "cover" / "models" / "kokoro-v1.0.onnx"
SMOKE_VOICES_PATH = ROOT / "narration_smoke" / "cover" / "models" / "voices-v1.0.bin"
...
def resolve_model_paths() -> tuple[Path, Path]:
    ...
    if SMOKE_MODEL_PATH.exists() and SMOKE_VOICES_PATH.exists():
        return SMOKE_MODEL_PATH, SMOKE_VOICES_PATH
    raise FileNotFoundError(
        "Missing Kokoro model files. Expected either narration/models/ or narration_smoke/cover/models/."
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate per-page narration audio for the PhD confirmation linked slides.")
