...
class S5SurfaceKinematicsSlides(Slide):
    ...
    def construct(self):
        ...
        for center, (heading, color) in zip(centers, headings):
            ...
            label = Text(heading, font_size=31, color=color, weight=BOLD).move_to(center + UP * 1.28)
    ...
    def directional_wave_group(self, center, phase):
        ...
        xy = VGroup(
            MathTex("x", font_size=20, color=C_MUTED).move_to(project(1.13, -0.70, 0)),
            MathTex("y", font_size=20, color=C_MUTED).move_to(project(-1.03, 0.80, 0)),
        )
    ...
    def time_series_visual(self, center, phase):
        ...
        labels = VGroup(
            Text("linear input", font_size=17, color=C_LINEAR).move_to(linear_center + RIGHT * 0.48 + UP * 0.40),
            Text("nonlinear output", font_size=15, color=C_ELEVATION).move_to(nonlinear_center + RIGHT * 0.36 + UP * 0.40),
        )
        relation = VGroup(
            Text("linear", font_size=16, color=C_LINEAR, weight=BOLD),
            Arrow(LEFT * 0.26, RIGHT * 0.26, buff=0, color=C_TIME, stroke_width=3.0, max_tip_length_to_length_ratio=0.28),
            Text("nonlinear", font_size=16, color=C_ELEVATION, weight=BOLD),
        ).arrange(RIGHT, buff=0.08)
        note = Text("add bound harmonics", font_size=11, color=C_MUTED)
    ...
    def inverse_creamer_visual(self, center, phase):
        ...
        labels = VGroup(
            Text("nonlinear input", font_size=15, color=C_ELEVATION).move_to(nonlinear_center + LEFT * 0.38 + UP * 0.40),
            Text("linear modes", font_size=15, color=C_LINEAR).move_to(linear_center + LEFT * 0.50 + UP * 0.40),
        )
        relation = VGroup(
            Text("nonlinear", font_size=16, color=C_ELEVATION, weight=BOLD),
            Arrow(LEFT * 0.26, RIGHT * 0.26, buff=0, color=C_INVERSE, stroke_width=3.0, max_tip_length_to_length_ratio=0.28),
            Text("linear modes", font_size=16, color=C_LINEAR, weight=BOLD),
        ).arrange(RIGHT, buff=0.08)
        note = Text("recover linear modes", font_size=11, color=C_MUTED)
        relation_group = VGroup(relation, note).arrange(DOWN, buff=0.03).move_to(center + DOWN * 1.01)
        relation_badge = semantic_badge(relation_group.get_center(), 2.16, 0.54, C_INVERSE, fill_opacity=0.06, stroke_opacity=0.65)
